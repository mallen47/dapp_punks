	...
	describe('Minting', () => {
		...
		describe('Failure', async () => {
			it('rejects insufficient payment', async () => {
				...
				await expect(nft.connect(minter).mint(1, { value: ether(1) }))
					.to.be.reverted;
			});

			it('rejects minting before allowed time', async () => {
				const ALLOW_MINTING_ON = new Date('May 26, 2035 18:00')
					.getTime()
					.toString()
					.slice(0, 10);
				const NFT = await ethers.getContractFactory('NFT');
			...
			it('rejects minting amount which exceeds supply', async () => {
				const ALLOW_MINTING_ON = Date.now().toString().slice(0, 10);
				const NFT = await ethers.getContractFactory('NFT');
				nft = await NFT.deploy(
					NAME,
				...
				await expect(nft.connect(minter).mint(24, { value: COST })).to
					.be.reverted;
			});

			it('limits minting to maximum of 5 tokens', async () => {
				const ALLOW_MINTING_ON = Date.now().toString().slice(0, 10);
				const NFT = await ethers.getContractFactory('NFT');
				nft = await NFT.deploy(
					NAME,
				...
				await expect(
					nft.connect(minter).mint(6, { value: COST.mul(6) })
				).to.be.revertedWith(
